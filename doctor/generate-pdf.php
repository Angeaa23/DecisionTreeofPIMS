...
$tbl_header = '<table border="0" class="table table-bordered" cellspacing="0" cellpadding="5">';
$tbl_footer = '</table>';
$tbl ='';
$tbl .= '
		<head>
		...
		<tr align="center">
		   <th colspan="4" align="left" style="font-size:14px;font-weight:bold;">Consultation History</th> 
		  </tr>
		';

$val=mysqli_query($con,"select * from tblmedicalhistory  where PatientID='$vid'  ORDER BY CreationDate DESC");
$num=mysqli_num_rows($val);
if($num>0){
$cnt = 1;
while ($row_val=mysqli_fetch_array($val)) {
	$patage = $row_val['PatientAge'];
	$weight = $row_val['Weight'];
	$height = $row_val['Height'];
	$bmi = $row_val['Bmi'];
	$bmicategory = $row_val['BmiCategory'];
	$waistcircum = $row_val['WaistCircum'];
	$birthweight = $row_val['BirthWeight'];
	$birthlength = $row_val['BirthLength'];
	$medhistory = $row_val['MedHistory'];
	$medicalpres = $row_val['MedicalPres'];
	$creationdate = $row_val['CreationDate'];

$tbl .= '		
		  <tr>
		    <th scope>Visit Date</th>
			<td colspan="4">' . $cnt = date('F j, Y, g:i a', strtotime($creationdate)) . '</td>
			<th scope></th>
			<td></td>
		  </tr>
		  ...
		  <tr>
			...
			<td>' . $patage . '</td>
			<th scope width="35%">Waist Circumference (in)</th>
			<td>' . $waistcircum . '</td>
		  </tr>
		  
		  <tr>
		    <th scope>Weight (kg)</th>
			<td>' . $weight . '</td>
			<th scope>Birth Weight (kg)</th>
			<td>' . $birthweight . '</td>
		  </tr>
		  
		  <tr>
			<th scope>Height (cm)</th>
			<td>' . $height . '</td>
			<th scope>Birth Length (cm)</th>
			<td>' . $birthlength . '</td>
		  </tr>
		  
		  <tr>
			<th scope>Body Mass Index</th>
			<td>' . $bmi . '</td>
			<th scope>Medical History</th>
			<td>' . $medhistory . '</td>
		  </tr>
		  
		  <tr>
			<th scope>BMI Category</th>
			<td>' . $bmicategory . '</td>
			<th scope>Medical Prescription</th>
			<td>' . $medicalpres . '</td>
		  </tr>
...
$pdf->writeHTMLCell(0, 0, 15, 20, $tbl_header . $tbl . $tbl_footer, 0, 1, 0, true, '', true);
